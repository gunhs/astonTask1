 ...
 * список).
 * <p>
 * Списки допускают дублирование элементов.  Более формально,
 * списки обычно допускают пары элементов {@code e1} и {@code e2}
 ...
 * доступа к элементам списка. Эта операция может выполняться во времени, пропорциональном значению индекса.
 * <p>
 * Интерфейс {@code MyList} предоставляет итератор {@code MyIterator}, который позволяет перемещаться по списку.
 * Предоставляется метод для получения
 * итератор списка, который начинается с указанной позиции в списке.
 * <p>
 ...
 * Во многих реализациях он будет выполнять дорогостоящий линейный поиск.
 * <p>
 * Интерфейс {@code MyList} предоставляет два метода для эффективной вставки и
 * удаления нескольких элементов в произвольной точке списка.
 * <p>
 *
