...
public class MyArrayList<E> implements MyList<E> {
     ...
     * Операции size, isEmpty, get, set выполняются за постоянное время.
     * Операция добавления выполняется за константу O(1),
     * за исключением тех случаев, когда происходит увеличение размера массива,
     * то есть для добавления n элементов требуется O(n) времени.
     * Операция remove выполняется за время O(n)
     * Все остальные операции выполняются за линейное время (грубо говоря).
     * Каждый экземпляр MyArrayList имеет начальный размер 10.
     * При достижении 10 элементов размер MyArrayList автоматически увеличивается ещё на 10.
     * Если несколько потоков обращаются к экземпляру MyArrayList одновременно, и по крайней мере один из потоков
     * структурно изменяет список, он должен быть синхронизирован извне.
     * Итератор, возвращаемый методами iterator потоконебезопасен.
     ...
     * Проверяет элементы по {@code Objects.equals(o, e)}. Сложность выполнения O(n)
     */
